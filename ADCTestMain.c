...
int main(void){ 
  PLL_Init(Bus80MHz);                   // 80 MHz
	...
	int selSamp=-1;												//selects ADC sampling rate
  ...
  //ADC0_InitSWTriggerSeq3_Ch9(2);      //uses 4 times sampling rate on the ADC
	...
	//ADC0_InitSWTriggerSeq3_Ch9(selSamp);	//uses 64 times sampling rate on the ADC
    ...
    GPIO_PORTF_DATA_R ^= 0x02;
		...
		if(dataFlag == 1){
			...
			switch(selSamp){
				...
				case 1: {
					ADC0_InitSWTriggerSeq3_Ch9(2);			//uses 1 times sampling rate on the ADC
					break;
				}
				case 2: {
					ADC0_InitSWTriggerSeq3_Ch9(4);			//uses 1 times sampling rate on the ADC
					break;
				}
				case 3: {
					ADC0_InitSWTriggerSeq3_Ch9(6);			//uses 1 times sampling rate on the ADC
					break;
				}
			}
			while(dataFlag == 0){};
			jitter=Jitter_Calc(ADCTime);
			PMF_Create(ADCData,pmf_array, selSamp);
			dataFlag++;
		}
  }
}
